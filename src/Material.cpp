...
double Material::EDF(double lambda, const LocalGeometry &local,
...
void Material::distribution(Angle outs[], const LocalGeometry &local,
        const Angle &in, unsigned int start, unsigned int stop,
        unsigned int jitx, unsigned int jity) const
{
    distribution(outs, local, start, stop, jitx, jity);
}
